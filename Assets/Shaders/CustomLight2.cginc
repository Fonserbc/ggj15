...
float3 computeBeat(float3 albedo, float3 light, float percentage) {
	float3 surfHSL = RGBtoHSL(albedo);
	float3 lightHSL = RGBtoHSL(light);
	surfHSL.x = torramod(surfHSL.x, 1.0f);
	lightHSL.x = torramod(lightHSL.x, 1.0f);
	float3 result = surfHSL;
	result.z = 0.4*percentage; //because yes, because we like gay pastel lights
	result.y = 1; //in soviet game jams, colors saturate you
	if(lightHSL.z > 0.5f && lightHSL.y != 0.0f) { //general light case
		float weight = min(1,(lightHSL.z-0.5f)/0.5f);
		float dist = (lightHSL.x >= surfHSL.x ? lightHSL.x - surfHSL.x : surfHSL.x - lightHSL.x); //hue distance
...
float4 LightingCustomLight2_PrePass (SurfaceOutput s, float4 light) {
	float3 col;
	if(s.Gloss < 0.4) col = computeCenter(s.Albedo, light.rgb);
	else if(s.Gloss < 0.8) col = computeBeat(s.Albedo, light.rgb, s.Specular);
	else col = computeBorder(s.Albedo, light.rgb);
	return float4(col,1);
}
